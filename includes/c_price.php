

    <div id="membership" class="row-back center">

      <div class="w-container wrapper">

        <div class="center">

          <h1 class="underline">BECOME A MEMBER TODAY!</h1>

          <p>
            Discover how a coworking space can help you work better.  These special introductory membership rates are only available for a limited time.
          </p>

        </div>

        <div class="pricing-table">

          <div class="w-col w-col-3  ">

            <div class="pricing-section">

              <div class="pricing-header">

                <h3>PART TIME MEMBERSHIP</h3>

                <span class="price"> Only $55 / Month</span>
              </div>

              <div class="pricing-content">

                <ul>

                  <li>
                    10 days a month
                  </li>

                  <li>
                    Access to conference/meeting room
                  </li>

                  <li>
                    Use any desk except reserved
                  </li>

                </ul>

              </div>

            </div>

          </div>

          <div class="w-col w-col-3  delay-05s">
            <div class="pricing-section">
              <div class="pricing-header">
                <h3>FULL-TIME MEMBERSHIP</h3>
                <span class="price">Only $100 / Month</span>
              </div>
              <div class="pricing-content">
                <ul>
                  <li>
                    24/7 Access
                  </li>
                  <li>
                    Use any desk that isn't reserved
                  </li>
                  <li>
                    Use of conference room
                  </li>
                </ul>
              </div>
            </div>
          </div>
          <div class="w-col w-col-3  delay-05s">
            <div class="pricing-section">
              <div class="pricing-header">
                <h3>FULL-TIME RESERVED</h3>
                <span class="price">Only $195 / Month</span>
              </div>
              <div class="pricing-content">
                <ul>
                  <li>
                    Same as coworker, but with a dedicated desk
                  </li>
                  <li>
                    Access to conference/meeting room
                  </li>
                  <li>
                    Limited Availability
                  </li>
                </ul>
              </div>
            </div>
          </div>
          <div class="w-col w-col-3 delay-1s">
            <div class="pricing-section">
              <div class="pricing-header">
                <h3>PRIVATE OFFICE</h3>

                <span class="price">Only $325 / Month</span>

              </div>

              <div class="pricing-content">

                <ul>
                  <li>24/7 Access</li>
                  <li>
                    Separate office with access to coworking space benefits.
                  </li>

                  <li>
                    Keyed entry to office.  It's all yours.
                  </li>

                </ul>

              </div>

            </div>

          </div>

        </div>
      </div>
      <a href="#contact" class="button member" onclick="ga('send', 'event', 'button', 'Click', 'become member c');">Join today!</a>

    </div>

    <!--///////////////////////////////////////////////////////

    // End Our-Plan section

    //////////////////////////////////////////////////////////-->
